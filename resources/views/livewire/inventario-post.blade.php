<div>
            ...
            <div class="bg-primary-25 p-4 rounded-lg">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div class="flex items-center gap-2 mb-5">
                        <select wire:model.live="stockFilter" class="bg-white border shadow-md border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-1.5">
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    <span class="font-medium">{{ $inventario->edition->book->titulo ?? 'N/A' }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <span class="font-semibold {{ $inventario->cantidad <= $inventario->umbral ? 'text-red-600' : 'text-gray-900' }}">
                                        {{ $inventario->cantidad }} unidades
                                    </span>
                                </td>

                                <td class="px-6 py-4">
                                    <span class="text-gray-600">{{ $inventario->umbral }} unidades</span>
                                </td>
            ...
            <div class="flex justify-between mt-3 gap-8 items-center">
                ...
                <div class="flex items-center gap-4 ml-4 mt-3">
                    <button wire:click="eliminarshowmodal"
                        class="px-4 py-3 mb-4 flex items-center justify-center font-semibold rounded-lg shadow-md transition-all duration-200 transform hover:scale-105 border-2 {{ count($selectedInventarios) >= 2 ? 'bg-white hover:bg-red-400 text-black border-black cursor-pointer' : 'bg-gray-300 text-gray-500 border-gray-300 cursor-not-allowed opacity-50' }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" wire:model="inventarioEditado.cantidad" required min="0"
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        @error('inventarioEditado.cantidad') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Umbral *</label>
                        <input type="number" wire:model="inventarioEditado.umbral" required min="0"
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        @error('inventarioEditado.umbral') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                </div>
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center z-50 pt-4">
        <div class="bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4">
            ...
            <p class="mb-6">¿Estás seguro de eliminar este inventario? Esta acción desasociará el inventario de la edición.</p>
            ...
            @else
            <p class="mb-6">¿Estás seguro de eliminar estos {{ count($selectedInventarios) }} inventarios? Esta acción desasociará los inventarios de sus ediciones.</p>
            <div class="flex justify-end space-x-4">
                <button wire:click="cancelarEliminacion"
                    class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400 transition-colors">
                    Cancelar
